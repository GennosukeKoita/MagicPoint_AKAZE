...
def export_detector_homoAdapt_gpu(config, output_dir, args):
    ...
    homoadapt_enable = config["data"]["homography_adaptation"]["enable"]
    ...
    with open(save_file, "a") as myfile:
        if homoadapt_enable:
            myfile.write("homography adaptation: " + str(homoadapt_enable) + "\n")
            myfile.write("homography adaptation_num_samples: " + str(homoAdapt_iter) + "\n")
