...
def sample_homography(
    ...
    if perspective:
        if not allow_artifacts:
            perspective_amplitude_x = min(perspective_amplitude_x, margin)
            perspective_amplitude_y = min(perspective_amplitude_y, margin)
        perspective_displacement = tf.compat.v1.truncated_normal([1], 0., perspective_amplitude_y/2)
        h_displacement_left = tf.compat.v1.truncated_normal([1], 0., perspective_amplitude_x/2)
        h_displacement_right = tf.compat.v1.truncated_normal([1], 0., perspective_amplitude_x/2)
    ...
    if scaling:
        scales = tf.concat(
                [[1.], tf.compat.v1.truncated_normal([n_scales], 1, scaling_amplitude/2)], 0)
        ...
        idx = valid[tf.compat.v1.random_uniform((), maxval=tf.shape(input=valid)[0], dtype=tf.int32)]
        pts2 = scaled[idx]
    ...
    if translation:
        ...
        if allow_artifacts:
            t_min += translation_overflow
            t_max += translation_overflow
        pts2 += tf.expand_dims(tf.stack([tf.compat.v1.random_uniform((), -t_min[0], t_max[0]),
                                         tf.compat.v1.random_uniform((), -t_min[1], t_max[1])]),
                               axis=0)

    # Random rotation
    # sample several rotations, check collision with borders, randomly pick a valid one
    if rotation:
        angles = tf.compat.v1.lin_space(tf.constant(-max_angle), tf.constant(max_angle), n_angles)
    ...
    p_mat = tf.transpose(a=tf.stack(
        [[pts2[i][j] for i in range(4) for j in range(2)]], axis=0))
    homography = tf.transpose(a=tf.compat.v1.matrix_solve_ls(a_mat, p_mat, fast=True))
